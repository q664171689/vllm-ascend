...
def test_kimi_k3_w4a8_shared_situ_uses_projection_grouped_matmuls(monkeypatch):
    ...
    situ_scale = torch.ones(2)
    expected = torch.randn(2, 2, dtype=torch.bfloat16)
    scoped_scheme = AscendKimiK3W4A8DynamicLinearMethod.__new__(AscendKimiK3W4A8DynamicLinearMethod)
    ...
    shared_experts.layer.act_fn = shared_experts.situ_activation
    shared_experts._has_a8_backend_provider, shared_experts._a8_backend = (
        shared_experts_module._create_shared_expert_a8_backend(shared_experts.layer)
    )
    dequant_situ_quant = MagicMock(return_value=(quantized_situ, situ_scale))
