...
from vllm_ascend.ops.fused_moe.dataclass.shared_experts import SharedExpertA8Backend
...
class AscendKimiK3W4A8DynamicLinearMethod(AscendLinearScheme):
    ...
    def create_shared_expert_a8_backend(self, layer: torch.nn.Module) -> SharedExpertA8Backend | None:
        for projection in (layer.gate_up_proj, layer.down_proj):
            scheme = getattr(getattr(projection, "quant_method", None), "quant_method", None)
            if not isinstance(scheme, AscendKimiK3W4A8DynamicLinearMethod) or not hasattr(projection, "weight_scale"):
                return None
        activation = layer.act_fn
        if not isinstance(activation, SituAndMul) or getattr(layer, "expert_gate", None) is not None:
            return None
        return _W4A8SharedExpertA8Backend(layer, activation)
...
class _W4A8SharedExpertA8Backend:
    """Single-group W4A8 projections and fused SiTU quantization, without scheduling."""

    def __init__(self, layer: torch.nn.Module, activation: SituAndMul) -> None:
        self.layer = layer
        self.activation = activation

    def gate_up(self, x: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
        return self.layer.gate_up_proj((x, scale))[0]

    def activation_quant(self, gate_up: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        # Grouped matmul already dequantized the INT4 projection to BF16.
        return torch.ops._C_ascend.dequant_situ_quant(
            ...
            quant_offset=None,
            group_index=None,
            beta=self.activation.beta,
            linear_beta=self.activation.linear_beta or 0.0,
            activate_left=True,
            quant_mode="dynamic",
        )

    def down(self, x: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
        return self.layer.down_proj((x, scale))[0]
